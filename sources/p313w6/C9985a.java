package p313w6;

import android.content.Context;
import android.content.pm.PackageInfo;
import android.content.pm.PackageManager;
import android.content.pm.Signature;
import android.util.Base64;
import java.io.ByteArrayInputStream;
import java.security.cert.CertificateException;
import java.security.cert.CertificateFactory;
import java.security.cert.X509Certificate;
import java.util.Arrays;
import p328x6.C10149w;

public final class C9985a {
    public static final byte[][] f32467a = {Base64.decode("", 0), Base64.decode("", 0), Base64.decode("", 0), Base64.decode("", 0), Base64.decode("", 0), Base64.decode("", 0)};

    public static boolean m6309a(PackageInfo packageInfo) {
        Signature[] signatureArr;
        try {
            ((X509Certificate) CertificateFactory.getInstance("X509").generateCertificate(new ByteArrayInputStream(packageInfo.signatures[0].toByteArray()))).checkValidity();
            signatureArr = packageInfo.signatures;
        } catch (CertificateException unused) {
        }
        if (signatureArr.length != 1) {
            return false;
        }
        byte[] byteArray = signatureArr[0].toByteArray();
        int i = 0;
        while (true) {
            byte[][] bArr = f32467a;
            if (i >= bArr.length) {
                break;
            } else if (Arrays.equals(bArr[i], byteArray)) {
                return true;
            } else {
                i++;
            }
        }
        return false;
    }

    public static EnumC9986b m6308b(Context context) {
        try {
            PackageManager packageManager = context.getPackageManager();
            String a = C10149w.m5896a(context);
            return !m6309a(packageManager.getPackageInfo(a, 64)) ? EnumC9986b.SERVICE_INVALID : C10149w.m5895b(context, a) ? EnumC9986b.SERVICE_VERSION_UPDATE_REQUIRED : !packageManager.getApplicationInfo(a, 0).enabled ? EnumC9986b.SERVICE_DISABLED : EnumC9986b.SUCCESS;
        } catch (PackageManager.NameNotFoundException unused) {
            return EnumC9986b.SERVICE_MISSING;
        }
    }
}
