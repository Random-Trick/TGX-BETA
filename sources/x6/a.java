package x6;

import android.content.Context;
import android.content.pm.PackageInfo;
import android.content.pm.PackageManager;
import android.content.pm.Signature;
import android.util.Base64;
import java.io.ByteArrayInputStream;
import java.security.cert.CertificateException;
import java.security.cert.CertificateFactory;
import java.security.cert.X509Certificate;
import java.util.Arrays;
import y6.w;

public final class a {
    public static final byte[][] f26060a = {Base64.decode("", 0), Base64.decode("", 0), Base64.decode("", 0), Base64.decode("", 0), Base64.decode("", 0), Base64.decode("", 0)};

    public static boolean a(PackageInfo packageInfo) {
        Signature[] signatureArr;
        try {
            ((X509Certificate) CertificateFactory.getInstance("X509").generateCertificate(new ByteArrayInputStream(packageInfo.signatures[0].toByteArray()))).checkValidity();
            signatureArr = packageInfo.signatures;
        } catch (CertificateException unused) {
        }
        if (signatureArr.length != 1) {
            return false;
        }
        byte[] byteArray = signatureArr[0].toByteArray();
        int i10 = 0;
        while (true) {
            byte[][] bArr = f26060a;
            if (i10 >= bArr.length) {
                break;
            } else if (Arrays.equals(bArr[i10], byteArray)) {
                return true;
            } else {
                i10++;
            }
        }
        return false;
    }

    public static b b(Context context) {
        try {
            PackageManager packageManager = context.getPackageManager();
            String a10 = w.a(context);
            return !a(packageManager.getPackageInfo(a10, 64)) ? b.SERVICE_INVALID : w.b(context, a10) ? b.SERVICE_VERSION_UPDATE_REQUIRED : !packageManager.getApplicationInfo(a10, 0).enabled ? b.SERVICE_DISABLED : b.SUCCESS;
        } catch (PackageManager.NameNotFoundException unused) {
            return b.SERVICE_MISSING;
        }
    }
}
